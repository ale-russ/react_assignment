...
const NewStoreEvent = () => {
  ...
  return (
    ...
    <section className="flex flex-col justify-between items-center w-[240px] space-y-56">
      <div>
        <h3 className="box-title"> {filteredItems[2]?.title}</h3>
        <p className="box-subtitle">{filteredItems[2]?.subtitle}</p>
      </div>
    ...
    >
      {filteredItems &&
        filteredItems[2]?.items.map((item, index) => {
          return (
            <div key={index}>
